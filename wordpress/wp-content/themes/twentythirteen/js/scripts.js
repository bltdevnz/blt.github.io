...
function terms() {$('#terms').fadeToggle('fast');}
...
function changeTab(n) {
	var c = [ document.getElementById('admin'), document.getElementById('videos'), document.getElementById('account') ];
...
function feedback() {
	...
	$.post( "feedback.php", { name: n, email: e, reason: s, message: m } ).done(function(data) {
...
function viewCat(p) {
	...
	$.ajax( {
		method: "GET",
		url: "http://malcam4me.kiwi/update.php?par=" + p,
		dataType: "html",
		success: updateData,
		async: false
	});
...
function doImportDB() {
	var p = new FormData($('form')[0]);
		...
		// client side. (this literally only reloads front page of site or says incorrect login)
	$.ajax( {
			url: "http://malcam4me.kiwi/import.php", 
			type: "POST",
			data: p,
			async: true
	});
...
function GetVideo(v,c, p) {
	$.ajax( {
		method: "GET",
		url: "http://malcam4me.kiwi/watch.php?vid=" + v + "&c=" + c,
		dataType: "html",
		success: showVid,
		async: false
	});
